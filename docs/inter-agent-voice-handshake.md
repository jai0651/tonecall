...
A hackathon idea for **Plivo For Agents**: when two AI voice agents end up on a call with each other, they detect each other and switch from slow speech-to-speech to a fast structured data channel — brokered by Plivo. Audibly reminiscent of fax/modem handshakes.
...
**This matters a lot for our design.** Plivo supports both; we'll require RFC 2833.

### 2.6 CPaaS
**Communications-Platform-as-a-Service** — companies like Plivo, Twilio. They own phone numbers, run SIP infrastructure, and expose APIs so developers can `POST /calls` instead of running their own telecom stack. Importantly, when your call goes through Plivo, **Plivo sees both ends** and can inject signals, record, conference, etc.

### 2.7 Voice AI
What Plivo (and others) now sell on top of the phone stack: managed STT + LLM + TTS pipelines you can drop into a call. You give it a system prompt; it answers your phone. This is the layer our "agent" lives in.

### 2.8 Webhooks
When something happens on the call (dial, answer, hangup, DTMF pressed), Plivo can `POST` to your server. This is how your code reacts to call events.

### 2.9 Historical precedent: modems and fax
Old modems sent digital data over voice phone lines by encoding bits as audio tones (FSK — frequency-shift keying). Fax machines do the same. The "screeching" sound at call start is the two devices negotiating speed and modulation. **We are reinventing this idea, but cheating: we'll use the audio channel only for "detection" and let the actual data flow over the internet via Plivo.**

---

## 3. The idea, in one paragraph

When a voice agent connects to a call, it emits a short DTMF preamble (`*0*` + a nonce). It listens for the same pattern from the other side. If both sides emit it, they each tell Plivo "I'm an agent, paired on call X, here's my nonce." Plivo verifies both nonces match the same call, then opens a WebSocket between the two agents. From then on, the agents exchange structured JSON over the WebSocket at internet speed, while the phone call stays open for humans / fallback. To make the demo memorable, we play stylized modem audio over the phone line during the JSON exchange so audiences hear "the agents are talking."
...
- Both agents are registered with Plivo's "agent registry": each has an `agent_id` and an HMAC signing key.
- Both agents run a small SDK that wraps the standard voice agent pipeline with handshake logic.

### Step 1: Call connects
Agent A's backend dials Plivo's API:
```
POST /v1/calls { from: "+1-A", to: "+1-B", answer_url: "https://a.com/answer" }
```
Plivo dials `+1-B`, which routes to Agent B's voice infra. Both sides are now in a normal SIP/RTP call. **At this point, neither agent knows the other side is an agent.**
...
Each agent is also *listening* for this pattern from the other side via DTMF webhooks from Plivo.
...
### Step 3: Pairing with Plivo (the trust step)
Both agents independently POST to Plivo's broker:
```
POST /v1/agent-pair
{
  "agent_id": "agent_A_id",
  "call_uuid": "call-uuid-from-plivo",
  "my_nonce": "12345678",
  "peer_nonce": "87654321",
  "hmac": "<signed hmac>"
}
...
- Are actually on the call (Plivo verifies),
...
Both agents now have an open WebSocket to Plivo's broker, which relays messages between them.

```
A → Plivo → B :  {"type": "hello", "capabilities": ["pizza_order_v1"]}
B → Plivo → A :  {"type": "hello", "capabilities": ["pizza_order_v1"]}
A → Plivo → B :  {"type": "intent", "name": "order_pizza", "slots": {
                    "size": "large", "toppings": ["pepperoni", "mushroom"],
                    "address": "...", "time": "ASAP"}}
B → Plivo → A :  {"type": "confirm", "order_id": "X742", "eta_minutes": 22,
                    "total_usd": 18.50}
A → Plivo → B :  {"type": "commit", "payment_token": "tok_..."}
B → Plivo → A :  {"type": "done"}
```
...
- A human picks up an extension and starts speaking (Plivo can detect speech energy).
...
### Step 7: Hangup
Either side sends `{"type": "bye"}` over WebSocket, both close their legs via Plivo's hangup API. Done.
...
Timeline:
  ...
  t=0.7s   both agents detect peer preamble (via Plivo DTMF webhook)
...
## 6. Concrete implementation with Plivo audio streaming
...
### 6.1 What Plivo `<Stream>` actually gives you

Plivo's `<Stream>` verb (equivalent of Twilio Media Streams) taps a live call's audio into a WebSocket you control:
...
Over that WebSocket:
- **Inbound (Plivo → you):** JSON frames with base64 μ-law 8kHz audio, ~20ms per chunk — what's being said on the call.
- **Outbound (you → Plivo):** JSON frames with base64 audio that Plivo injects into the call as if you were speaking.
...
```
1. PLIVO CLOUD            — owns +1-A and +1-B, runs the actual phone call
2. YOUR ANSWER URL         — HTTP endpoint, replies with <Stream> XML
3. YOUR VOICE WS HANDLER   — receives audio from Plivo, sends audio back
...
Boxes 2–7 all live on **your infra** — one FastAPI app is fine for the hackathon. Only box 1 is Plivo.
...
│   WS   /voice/{call_uuid}   → bidirectional audio with Plivo    │
...
**t = 0.1s — Plivo sets up TWO call legs.** PSTN has no concept of "agent-to-agent." Plivo bridges:
- **Leg A** (outbound from +1-A): Plivo POSTs your outbound answer URL.
- **Leg B** (inbound to +1-B): Plivo POSTs your inbound answer URL.

Each leg has its **own** `call_uuid`. They're related (same bridged call) but separate from your code's point of view.

**t = 0.2s — both legs answered with `<Stream>`.** Both answer URLs return the `<Stream>` XML. Plivo opens **two** voice WebSockets to your service:
...
async def voice_ws(ws, call_uuid):
    ...
    plivo_client.calls.send_digits(call_uuid, digits=f"*0*{nonce}#")
...
**t = 0.4s — DTMF preambles cross.** Handler A injects `*0*11111111#` into leg A → audible on leg B (Plivo delivers it as `dtmf` events on leg B's voice WebSocket). Same in reverse.
...
**t = ~3s — done.** Either side sends `{"type":"bye"}`. Both handlers tear down their legs via `plivo_client.calls.delete(call_uuid)`.
...
This is just a normal voice AI built on Plivo Streams. The handshake never engaged; the same vegetable-vendor prompt drives an STT→LLM→TTS loop. The 1.5s of preamble beeps at start can be suppressed for inbound-from-non-agent calls in production. For hackathon, live with them.
...
1. **How does it physically work?** Plivo's `<Stream>` is the wire. Bidirectional WebSocket carrying audio + DTMF events. You receive audio, you send audio. That's the entire mechanism.
2. **Where does the agent live?** In your middleware. The prompt is a config-dict entry keyed by phone number. The LLM is a library call (OpenAI API). Plivo doesn't run your agent — Plivo just delivers the call audio to you.
3. **What does the data WebSocket do?** A **separate** WebSocket, nothing to do with Plivo. Your broker (also your infra) accepts connections from paired middlewares and relays JSON. The voice WebSocket continues in parallel for fallback or cosmetic modem audio.
...
A FastAPI starting point lives next to this doc: [`inter-agent-handshake-skeleton.py`](./inter-agent-handshake-skeleton.py). It implements the full state machine with Plivo + LLM + STT + TTS calls stubbed out. Drop in your Plivo creds and an API key for the LLM and it boots.
...
### 7.1 Plivo number provisioning + answer URL
Standard Plivo setup. Two numbers, two answer URLs. ~30 min.
...
### 7.3 Plivo broker (FastAPI server)
...
|---|---|---|
| Dial + connect two numbers via Plivo | ✅ Production-stable | Plivo's day job. |
| Send DTMF in-call | ✅ Production-stable | Plivo `POST /play_dtmf` API. |
| Receive DTMF webhook | ✅ Production-stable | Plivo `DTMF` callback. |
| HMAC nonce auth | ✅ Cryptographically sound | Standard primitive. |
| Open WebSocket from agent to broker | ✅ Trivial | FastAPI/Plivo Voice both support. |
| JSON over WebSocket relay | ✅ Trivial | A 30-line Python relay works. |
| Play stylized audio in-call | ✅ Production-stable | Plivo `Play` verb. |
...
2. **DTMF reliability on weird carrier paths.** If one leg is a satellite phone in Antarctica, the DTMF might not survive. Mitigation: insist on RFC 2833 (out-of-band DTMF), which Plivo supports. For PSTN legs that don't, fall back to spoken codeword detection ("AGENTPROTOZERO"). This degrades gracefully.
...
4. **Adoption** — only useful when *both* sides use the protocol. For a hackathon demo this is fine (we control both). For a real product, this is the "fax machine bootstrap" problem: it's most valuable when many systems support it. Plivo is uniquely positioned to seed it (they could enable it by default for any voice agent built on their platform).
...
- **Skipping the trust broker.** Without Plivo (or an equivalent broker) verifying both legs and matching nonces, the protocol is spoof-able. Don't.
- **Cross-platform out of the box.** If the other side is on a non-Plivo agent platform, they'd need to implement the spec too. Make the spec open ("Agent Voice Interop Protocol v0") so this is a path forward, not a roadblock.
...
- **CPaaS** — Plivo / Twilio; sells phone numbers + SIP + APIs.
...
- **Plivo Handshake** — boring but clear
